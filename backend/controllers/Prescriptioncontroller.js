...
const createPrescription = async (req, res) => {
    try {
        const { patientId, doctorId, appointmentId, diagnosisIds = [], medications, notes } = req.body;
        ...
        if (diagnosisIds.length > 0) {
            const foundDiagnoses = await Diagnosis.find({ _id: { $in: diagnosisIds } });
            if (foundDiagnoses.length !== diagnosisIds.length) {
                return res.status(400).json({
                    success: false,
                    message: 'One or more diagnosisIds do not exist in the Diagnosis catalog (BR-RX-004)',
                });
            }
        }
        ...
        const foundMedications = await Medication.find({ _id: { $in: medicationIds } });
        if (foundMedications.length !== medicationIds.length) {
            return res.status(400).json({
                success: false,
                message: 'One or more medicationIds do not exist in the Medication catalog (BR-RX-005)',
            });
        }
        ...
        if (error.code === 11000) {
            return res.status(409).json({
                success: false,
                message: 'A prescription already exists for this appointment (BR-RX-002)',
            });
        }
